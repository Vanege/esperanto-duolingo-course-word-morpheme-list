# esperanto-duolingo-course-word-morpheme-list
List of words (named "lexemes") and morphemes (roots, radicals, affixes, endings...) from the Duolingo course to teach Esperanto from English.

You can use these lists to:
- build your own Anki deck (to memorize them faster)
- write texts that are easy to read for people who learn Esperanto using Duolingo. For example, this tool to check the roots of an Esperanto text uses the list here: https://codepen.io/VanegeEsperanto/full/emgRbYv
  Duolingo is still the most popular way to learn Esperanto (as of 2026) despite a dramatic drop in quality due to the removal of essential features (grammar notes, most output exercises and the ability to ask for help for each sentence (forum)).

The two files:
- The file with the 2018 "lexemes" (actually they are the words that are explicitly taught by Duolingo) is based on https://duome.eu/vocabulary/en/eo, on the date of 2026-06-19. The numerical numbers (1887, 1905) are removed, and the proper nouns (Parizo, Toronto, Roterdam, IJK, UK...) are capitalized. Note that I manually added "ju" and "des" since they are not present in the original list, but they are clearly taught by Duolingo: https://duome.eu/tips/en/eo
- The file with the 1379 "morphemes" (aka roots or radicals) is based on the previous file. Words were divided manually. You should interpret the word "morpheme" as a translation of the word "radiko" in Esperanto, so the elements we can reuse to create new words.

I made the following decisions when dividing words into morphemes:
- correlatives are untouched (so "kiel" is considered one morpheme, but "ki" and "el" are not, as they are not part of the productive Esperanto word-building.)
- "-n" or "-j" on a correlative is not considered part of the correlative (so "tiun" is not a morpheme, but "tiu" and "n" are.)
- non-productive pseudo-affixes (-al) are not considered morphemes (they don't appear alone and derived words are semantically too unpredictable)
- the -i pseudo-affix for some countries is not considered a morpheme (so "franc" and "franci" are two morphemes)
- "paĉj" and "panj" are morphemes since "ĉj" and "nj" corrupt the roots they are attached to.
- "ĉj" and "nj" are not morphemes (they are not productive enough, it makes more sense to interpret ĉj-words and nj-words as having their own roots)
- "komunum", "respondec" and "ridet" are considered to be morphemes on their own since their meaning can't be deduced easily enough from their parts
